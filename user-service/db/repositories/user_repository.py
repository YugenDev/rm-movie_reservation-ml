...
class UserRepository:
    ...
    def delete_user(self, db: Session, user_id: str):
        user = db.query(User).filter(User.user_id.as_string(binary=True) == user_id).first()
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="aaaaaaaaaaaaaaaaaaaaaa")
        try:
            db.delete(user)
            db.commit()
        except SQLAlchemyError as e:
